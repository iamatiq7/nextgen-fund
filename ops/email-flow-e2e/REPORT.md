...
| member request stored on the record, status pending | PASS | id u-gNrvXr1J7Edb · fields ["email"] |
...
| decision kept in the permanent history | PASS | {"id":"u-gNrvXr1J7EdbfZe6THTQgJ5coyv1","fields":["email"],"from":"ngf-e2e-mua0zkn5@mailinator.com","to":"ngf-e |
| in-app notice written for the member | PASS | approved |
...
| alias document created for the new address (login before the link is opened) | PASS | {"uid":"gNrvXr1J7EdbfZe6THTQgJ5coyv1","type":"email","login":"ngf-e2e-mua0zkn5@mailinator. |
| old address marked retired | PASS | {"uid":"gNrvXr1J7EdbfZe6THTQgJ5coyv1","type":"email","retired":true,"movedTo":"ngf-e2e-mua |
| a second approval is refused (not-found) | PASS | not-found |
| login with the NEW address works | PASS | uid gNrvXr1J |
| login with the USERNAME still works (no lock-out) | PASS | uid gNrvXr1J |
| the OLD address is refused with a clear message (retired) | PASS | retired |
| confirmation mail requested from the member session ({"status":"sent","to":"ngf-e2e-mua0zkn5.new@mailinator.com"}) | PASS | {"status":"sent","to":"ngf-e2e-mua0zkn5.new@mailinator.com"} |
| the send is recorded on the record for the status banner | PASS | {"to":"ngf-e2e-mua0zkn5.new@mailinator.com","status":"sent","sentAt":"2026-09-20T16:23:46.036Z","requestedBy":"member-session"} |
| reconcile keeps the approved address while the link is pending | PASS | returned {"from":"ngf-e2e-mua0zkn5.new@mailinator.com","to":"ngf-e2e-mua0zkn5@mailinator.com","pending":"ngf-e2e-mua0zkn5.new@mailinator.com","applied":false} · record ngf***@mailinator.com |
...
| the rejection is recorded with its status | PASS | {"id":"u-gNrvXr1J7EdbfZe6THTQgJ5coyv1","fields":["email"],"from":"ngf-e2e-mua0zkn5.new@mailinator.com","to":"n |
